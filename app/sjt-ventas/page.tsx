...
export default function SjtVentasPage() {
  ...
  async function cargarItems() {
    const { data, error } = await supabase
      .from('items').select('*')
      .eq('test_id', 'a7b8c9d0-e1f2-3456-abcd-777777777777')
      .order('orden')
  ...
  async function terminarTest(todasLasRespuestas: Record<string, string>, todosLosItems: Item[]) {
    ...
    const { data: sesion, error } = await supabase.from('sesiones').insert({
      test_id: 'a7b8c9d0-e1f2-3456-abcd-777777777777',
    ...
    <div style={s.contenedor}>
      <div style={s.encabezado}>
        <div style={s.encabezadoTop}>
          <span style={s.testNombre}>SJT — Ventas Telefónicas</span>
...
const s = {
  ...
  testNombre: { fontSize: '0.75rem', fontWeight: '500', color: '#16a34a', textTransform: 'uppercase' as const, letterSpacing: '0.05em' } as React.CSSProperties,
  ...
  badge: { fontSize: '10px', padding: '2px 8px', borderRadius: '99px', background: '#EAF3DE', color: '#27500A', fontWeight: '500' } as React.CSSProperties,
  barraFondo: { width: '100%', height: '6px', background: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' } as React.CSSProperties,
  barraRelleno: { height: '100%', background: '#16a34a', borderRadius: '3px', transition: 'width 0.3s ease' } as React.CSSProperties,
  escenarioBox: { background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '1.25rem' } as React.CSSProperties,
  escenarioLabel: { fontSize: '10px', fontWeight: '600', color: '#166534', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '6px' } as React.CSSProperties,
