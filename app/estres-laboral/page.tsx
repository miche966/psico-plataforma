...
export default function EstresLaboralPage() {
  ...
  if (finalizado) return (
    <div style={s.contenedor}>
      ...
      <p style={s.mensajeConfirmacion}>
        Tus respuestas fueron registradas correctamente. El equipo de Recursos Humanos analizará los resultados de forma confidencial.
      </p>
      <div style={s.contactoBox}>
        <p style={s.contactoTitulo}>Confidencialidad</p>
        <p style={s.contactoTexto}>
          Esta evaluación tiene fines de diagnóstico organizacional. Los resultados individuales son confidenciales y solo se utilizarán de forma agregada para identificar áreas de mejora en el ambiente laboral.
        </p>
        <p style={s.contactoTexto}>Si tenés consultas podés comunicarte:</p>
  ...
  const coloresFactores: Record<string, string> = {
    carga_laboral: '#185FA5',
    relaciones: '#16a34a',
    claridad_rol: '#9333ea',
    equilibrio: '#ea580c',
    burnout: '#dc2626'
  ...
  const colorFactor = coloresFactores[item.factor] || '#185FA5'

  return (
    <div style={s.contenedor}>
      <div style={s.encabezado}>
        <div style={s.testNombre}>Evaluación de Estrés Laboral</div>
        ...
        <div style={s.barraFondo}>
          <div style={{ ...s.barraRelleno, width: `${progreso}%`, background: colorFactor }} />
        </div>
      </div>

      <div style={s.instruccion}>
        ¿Con qué frecuencia experimentás esta situación en tu trabajo?
      </div>
      ...
      <div style={s.opciones}>
        {item.opciones.map((opcion: string, index: number) => (
          <button
            ...
            onMouseEnter={e => {
              (e.target as HTMLButtonElement).style.background = colorFactor
              ;(e.target as HTMLButtonElement).style.color = '#fff'
              ;(e.target as HTMLButtonElement).style.borderColor = colorFactor
            }}
            onMouseLeave={e => {
...
const s = {
  ...
  testNombre: { fontSize: '0.75rem', fontWeight: '500', color: '#185FA5', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '0.5rem' } as React.CSSProperties,
  ...
  barraRelleno: { height: '100%', borderRadius: '3px', transition: 'width 0.3s ease' } as React.CSSProperties,
  instruccion: { fontSize: '12px', color: '#64748b', marginBottom: '0.75rem', fontStyle: 'italic' } as React.CSSProperties,
  ...
  contactoTexto: { fontSize: '0.875rem', color: '#64748b', lineHeight: '1.6', margin: '0 0 0.75rem' } as React.CSSProperties,
