...
export default function SjtAtencionPage() {
  ...
  async function cargarItems() {
    const { data, error } = await supabase
      .from('items').select('*')
      .eq('test_id', 'f6a7b8c9-d0e1-2345-fabc-666666666666')
      .order('orden')
  ...
  async function terminarTest(todasLasRespuestas: Record<string, string>, todosLosItems: Item[]) {
    ...
    const { data: sesion, error } = await supabase.from('sesiones').insert({
      test_id: 'f6a7b8c9-d0e1-2345-fabc-666666666666',
    ...
    <div style={s.contenedor}>
      <div style={s.encabezado}>
        <div style={s.encabezadoTop}>
          <span style={s.testNombre}>SJT — Atención al Cliente</span>
...
const s = {
  ...
  badge: { fontSize: '10px', padding: '2px 8px', borderRadius: '99px', background: '#E6F1FB', color: '#0C447C', fontWeight: '500' } as React.CSSProperties,
  ...
  escenarioBox: { background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '1.25rem' } as React.CSSProperties,
  escenarioLabel: { fontSize: '10px', fontWeight: '600', color: '#1E40AF', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '6px' } as React.CSSProperties,
