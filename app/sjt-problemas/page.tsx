...
export default function SjtProblemasPage() {
  ...
  async function cargarItems() {
    const { data, error } = await supabase
      .from('items').select('*')
      .eq('test_id', 'f2a3b4c5-d6e7-8901-fabc-222333444555')
      .order('orden')
    ...
    <div style={s.contenedor}>
      <div style={s.encabezado}>
        <div style={s.encabezadoTop}>
          <span style={s.testNombre}>SJT Resolución de Problemas</span>
...
const s = {
  ...
  testNombre: { fontSize: '0.75rem', fontWeight: '500', color: '#9333ea', textTransform: 'uppercase' as const, letterSpacing: '0.05em' } as React.CSSProperties,
  ...
  badge: { fontSize: '10px', padding: '2px 8px', borderRadius: '99px', background: '#F3EFFE', color: '#6D28D9', fontWeight: '500' } as React.CSSProperties,
  barraFondo: { width: '100%', height: '6px', background: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' } as React.CSSProperties,
  barraRelleno: { height: '100%', background: '#9333ea', borderRadius: '3px', transition: 'width 0.3s ease' } as React.CSSProperties,
  escenarioBox: { background: '#F3EFFE', border: '1px solid #DDD6FE', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '1.25rem' } as React.CSSProperties,
  escenarioLabel: { fontSize: '10px', fontWeight: '600', color: '#4C1D95', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '6px' } as React.CSSProperties,
