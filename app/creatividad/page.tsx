...
export default function CreatividadPage() {
  ...
  async function calcularResultado(todasLasRespuestas: Respuesta[]) {
    ...
    const response = await fetch('/api/evaluacion/public-data', {
      ...
      body: JSON.stringify({ action: 'finalize', candidato_id: candidatoId, proceso_id: procesoId, token, test_id: TEST_ID, puntaje_bruto: promedios, respuestas: todasLasRespuestas.map(r => ({ ...r, tiempo_respuesta: 0 })) })
    })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok) console.error('Error al guardar Creatividad:', payload.error)
  }
  ...
  const coloresFactores: Record<string, string> = {
    pensamiento_divergente: '#9333ea',
    flexibilidad: '#2563eb',
    innovacion: '#16a34a',
    tolerancia_ambiguedad: '#ea580c',
    curiosidad: '#db2777'
  }

  const colorFactor = coloresFactores[item.factor] || '#9333ea'
...
const s = {
  ...
  testNombre: { fontSize: '0.75rem', fontWeight: '500', color: '#9333ea', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: '0.5rem' } as React.CSSProperties,
